...
class Orchestrator:
    ...
    def run_herb(self, user_input: str) -> Dict[str, Any]:
        ...
        entity = self.entity_extractor.extract(user_input=q0, rag_hint=pre_hint)
        query = entity or q0
        ...
        evidence = clean_evidence_for_entity(evidence_raw, entity or query)
        ...
        card = self.card_agent.run(query, kg if kg.get("found") else None, evidence)
        narration = self.style_agent.run(card)
        persona = self.persona_agent.run(card, narration)

        tts = {"type": "tts", "ok": False}
        try:
            tts = self.tts_agent.run(card.get("name", query), narration, persona)
        except Exception as e:
            tts = {"type": "tts", "ok": False, "error": str(e)}

        image = {"type": "image", "ok": False}
        try:
            image = self.image_agent.run(card.get("name", query), persona)
        except Exception as e:
